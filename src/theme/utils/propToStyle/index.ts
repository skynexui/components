...
import { CSSInternalAPIPropNames } from "@src/theme/types/CSSProperties";
import { parseValue } from "./parseValue";
import { PLATFORM_MOBILE } from "@src/theme/types/Platforms";
  ...
  return (props: CSSPropValues): any => {
    ...
    if (typeof propValue === "object" && !Array.isArray(propValue)) {
      if (theme.platform === PLATFORM_MOBILE) {
        ...
        return {
          [propNameFormated]: parseValue(
            theme,
            propNameReceived,
            valueAdapter(validValueInBreapointOrder)
          ),
        };
      }
